  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (res.ok && data.paymentId) {
        window.location.href = `/reservation/${reservationId}/payment?paymentId=${encodeURIComponent(data.paymentId)}`
        return
      }
    } catch (_) {}
    setLoading(false)
...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    const fetchQuote = async () => {
      try {
        ...
        const data = await res.json()
        if (!res.ok) {
          throw new Error(data.message || "No se pudo calcular la tarifa")
        }
        setQuote(data)
      } catch (e: any) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error((data as any).message || "No se pudo crear la reserva")
      }
      ...
      if (payRes.ok && payData.paymentId) {
        window.location.href = `/reservation/${created.id}/payment?paymentId=${encodeURIComponent(payData.paymentId)}`
        return
      }
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="max-w-3xl mx-auto px-4 py-10 space-y-6">
        ...
        <section className="rounded-lg border bg-white p-4 space-y-2 text-sm">
          <p>
            <span className="font-medium">Vehículo:</span> {vehicleId}
          </p>
          <p>
            <span className="font-medium">Recogida:</span> {pickup}
          </p>
          <p>
            <span className="font-medium">Devolución:</span> {dropoff}
          </p>
          {loadingQuote ? (
            ...
            <div className="space-y-1">
              ...
              {quote.extrasBreakdown?.length ? (
                <>
                  {quote.extrasBreakdown.map((x) => (
                    <p key={x.extraId}>
                      {x.name} x{x.quantity}: {quote.currency} {x.price.toFixed(0)}
                    </p>
                  ))}
              ...
              <p>
                <span className="font-medium">Total estimado:</span> {quote.currency} {quote.totalPrice.toFixed(0)}
              </p>
            </div>
          ) : null}
        </section>
        ...
        {success ? (
          <section className="rounded-lg border border-green-200 bg-green-50 p-4 text-sm">
            ...
            <p>
              Número de reserva:{" "}
              <span className="font-mono font-bold text-green-900">{success.reservationNumber}</span>
            </p>
            <p>
              Importe: {success.currency} {success.totalPrice.toFixed(0)}
            </p>
            <p className="mt-2 text-xs text-slate-600">
              En una fase posterior, aquí se integrará el pago online y el envío de email/WhatsApp.
            </p>
            <div className="mt-3 flex flex-wrap gap-3">
              <a href={`/reservation/${success.id}`} className="inline-block rounded-md bg-teal-600 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-700">
                Ver mi reserva
              </a>
              <PayNowButton reservationId={success.id} provider={provider} />
            </div>
          </section>
        ) : (
