...
type Reservation = {
  ...
  vehicle: { id: string; brand: string; model: string; year: number | null; basePricePerDay?: number; location?: { name: string; code: string | null }; images?: { url: string }[] }
...
export default function ReservationDetailPage() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (paymentId) {
        window.location.href = `/${locale}/reservation/${id}/payment?paymentId=${encodeURIComponent(paymentId)}`
        return
      }
  ...
  const depositAmount =
    reservation.paymentStatus === "paid" && reservation.vehicle?.basePricePerDay
      ...
      <section className="max-w-2xl mx-auto px-4 py-10 space-y-6">
        ...
        <div className="rounded-lg border bg-white p-4 space-y-3 text-sm">
          ...
          <div className="mt-2 border-t border-slate-100 pt-2 space-y-1">
            <p className="font-medium text-slate-700">Estado de cuenta</p>
            <p>
              <span className="text-slate-600">Pagado (primer día):</span>{" "}
              {reservation.paymentStatus === "paid"
                ? `${reservation.currency} ${depositAmount.toFixed(0)}`
                : "—"}
            </p>
            <p>
              <span className="text-slate-600">Pendiente por pagar:</span>{" "}
              {reservation.currency} {remainingAmount.toFixed(0)}
            </p>
          </div>
        </div>
