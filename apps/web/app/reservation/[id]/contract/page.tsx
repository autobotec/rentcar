    ...
    <main className="min-h-screen bg-white p-6 md:p-10 print:p-6">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-4 text-sm mb-8">
          <p>
            <strong>Número de reserva:</strong> {reservation.reservationNumber}
          </p>
          <p>
            <strong>Cliente:</strong> {reservation.user.name || reservation.user.email}
          </p>
          <p>
            <strong>Email:</strong> {reservation.user.email}
          </p>
          <p>
            <strong>Vehículo:</strong> {reservation.vehicle.brand} {reservation.vehicle.model}
            {reservation.vehicle.year != null && `, año ${reservation.vehicle.year}`}
          </p>
          <p>
            <strong>Lugar y fecha de recogida:</strong> {reservation.pickupLocation.name} —{" "}
            {pickupDate.toLocaleString("es")}
          </p>
          <p>
            <strong>Lugar y fecha de devolución:</strong> {reservation.dropoffLocation.name} —{" "}
            {dropoffDate.toLocaleString("es")}
          </p>
          <p>
            <strong>Duración:</strong> {nights} día{nights !== 1 ? "s" : ""}
          </p>
          {reservation.extras?.length ? (
            <p>
              <strong>Extras:</strong>{" "}
              {reservation.extras.map((e) => `${e.extra.name} (x${e.quantity})`).join(", ")}
            </p>
          ) : null}
          <p>
            <strong>Importe total:</strong> {reservation.currency} {reservation.totalPrice.toFixed(2)}
          </p>
        </div>
